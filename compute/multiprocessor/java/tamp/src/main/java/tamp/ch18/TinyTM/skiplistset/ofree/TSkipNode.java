...
package tamp.ch18.TinyTM.skiplistset.ofree;

import tamp.ch18.TinyTM.AtomicArray;
import tamp.ch18.TinyTM.AtomicObject;
import tamp.ch18.TinyTM.exceptions.AbortedException;
import tamp.ch18.TinyTM.ofree.FreeObject;
import tamp.ch18.TinyTM.skiplistset.SSkipNode;
import tamp.ch18.TinyTM.skiplistset.SkipNode;
...
public class TSkipNode<T> implements SkipNode<T> {
  ...
  public TSkipNode(int level) {
    atomic = new FreeObject<SSkipNode<T>>(new SSkipNode<T>(level));
  }
  public TSkipNode(int level, int key, T item){
    atomic = new FreeObject<SSkipNode<T>>(new SSkipNode<T>(level, key, item));
  }
