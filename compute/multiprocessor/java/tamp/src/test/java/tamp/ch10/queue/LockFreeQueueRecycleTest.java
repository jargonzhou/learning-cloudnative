 ...
 */
public class LockFreeQueueRecycleTest extends TestCase {
  private final static int THREADS = 8;
  private final static int TEST_SIZE = 512;
  private final static int PER_THREAD = TEST_SIZE / THREADS;
  int index;
  LockFreeQueueRecycle<Integer> instance;
  boolean[] map = new boolean[TEST_SIZE];
  Thread[] thread = new Thread[THREADS];
  
  public LockFreeQueueRecycleTest(String testName) {
    super(testName);
    instance = new LockFreeQueueRecycle<Integer>();
  }
  
  public static Test suite() {
    TestSuite suite = new TestSuite(LockFreeQueueRecycleTest.class);
    return suite;
  }
