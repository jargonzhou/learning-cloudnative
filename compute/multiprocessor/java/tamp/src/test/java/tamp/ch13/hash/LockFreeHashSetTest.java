 ...
 * @author mph
 */
public class LockFreeHashSetTest extends TestCase {
  private final static int THREADS = 8;
  private final static int TEST_SIZE = 512;
  private final static int PER_THREAD = TEST_SIZE / THREADS;
  int index;
  LockFreeHashSet<Integer> instance;
  Thread[] thread = new Thread[THREADS];

  public LockFreeHashSetTest(String testName) {
    super(testName);
    instance = new LockFreeHashSet<Integer>(TEST_SIZE);
  }

  public static Test suite() {
    TestSuite suite = new TestSuite(LockFreeHashSetTest.class);
    return suite;
  }
