 ...
 * @author mph
 */
public class RefinableHashSetTest extends TestCase {
  private final static int THREADS = 8;
  private final static int TEST_SIZE = 512;
  private final static int PER_THREAD = TEST_SIZE / THREADS;
  int index;
  RefinableHashSet<Integer> instance;
  Thread[] thread = new Thread[THREADS];

  public RefinableHashSetTest(String testName) {
    super(testName);
    instance = new RefinableHashSet<Integer>(TEST_SIZE);
  }

  public static Test suite() {
    TestSuite suite = new TestSuite(RefinableHashSetTest.class);
    return suite;
  }
