 ...
 * @author mph
 */
public class RevBarrierTest extends TestCase {
  static final int THREADS = 8;   // number threads to test
  static final int ROUNDS = 8;    // how many rounds to test
  
  public RevBarrierTest(String testName) {
    super(testName);
  }
  
  public static Test suite() {
    TestSuite suite = new TestSuite(RevBarrierTest.class);
    
    return suite;
  }
  
  /**
   * Test of await method, of class barrier.RevBarrier.
   */
  public void testBarrier() { 
    System.out.format("Testing %d threads, %d rounds\n", THREADS, ROUNDS); 
    RevBarrier instance = new RevBarrier(THREADS, 2);
