 ...
 * @author mph
 */
public class CoarseHashSetTest extends TestCase {
  private final static int THREADS = 8;
  private final static int TEST_SIZE = 512;
  private final static int PER_THREAD = TEST_SIZE / THREADS;
  int index;
  CoarseHashSet instance;
  Thread[] thread = new Thread[THREADS];

  public CoarseHashSetTest(String testName) {
    super(testName);
    instance = new CoarseHashSet<Integer>(TEST_SIZE);
  }

  public static Test suite() {
    TestSuite suite = new TestSuite(CoarseHashSetTest.class);
    return suite;
  }
