 ...
 * @author mph
 */
public class CoarseCuckooHashSetTest extends TestCase {
  private final static int THREADS = 8;
  private final static int TEST_SIZE = 512;
  private final static int PER_THREAD = TEST_SIZE / THREADS;
  int index;
  CoarseCuckooHashSet<Integer> instance;
  Thread[] thread = new Thread[THREADS];

  public CoarseCuckooHashSetTest(String testName) {
    super(testName);
    instance = new CoarseCuckooHashSet<Integer>(2 * TEST_SIZE);
  }

  public static Test suite() {
    TestSuite suite = new TestSuite(CoarseCuckooHashSetTest.class);
    return suite;
  }
  ...
  public void testSequential() {
    ...
    for (int i = 0; i < TEST_SIZE; i++) {
      if (!instance.contains(i)) {
        fail("sequential: bad contains: " + i );
      }
    }
    for (int i = 0; i < TEST_SIZE; i++) {
    ...
    public void run() {
      for (int i = 0; i < PER_THREAD; i++) {
        if (!instance.remove(value+i)) {
          fail("DeqThread: duplicate remove");
        }
      }
    }
  }
