...
import com.spike.vertx.eventbus.rpc.SensorDataService;
import java.util.List;
import io.vertx.core.Vertx;
...
public class SensorDataServiceVertxEBProxy implements SensorDataService {
  ...
  public void sensors(Handler<AsyncResult<List<String>>> handler){
    ...
    _deliveryOptions.addHeader("action", "sensors");
    _deliveryOptions.getHeaders().set("action", "sensors");
  ...
  public void valueFor(String sensorId, Handler<AsyncResult<JsonObject>> handler){
    ...
    _deliveryOptions.getHeaders().set("action", "valueFor");
  ...
  public void average(Handler<AsyncResult<JsonObject>> handler){
    ...
    _deliveryOptions.getHeaders().set("action", "average");
    _vertx.eventBus().<JsonObject>request(_address, _json, _deliveryOptions, res -> {
      if (res.failed()) {
        handler.handle(Future.failedFuture(res.cause()));
      } else {
