...
import com.spike.eventstreams.samza.model.WikipediaParser;
import com.spike.eventstreams.samza.system.WikipediaFeed;
...
// NOTE: change to read from Kafka topic 'wikipedia-raw'.

/**
 * This {@link StreamApplication} demonstrates the Samza fluent API by performing the same operations as
 * {@literal WikipediaFeedStreamTask},
 * {@literal WikipediaParserStreamTask}, and
 * {@literal WikipediaStatsStreamTask} in one expression.
 *
...
//        .withChannel(WIKINEWS_CHANNEL);
    ...
    KafkaInputDescriptor<WikipediaFeed.WikipediaFeedEvent> rawInputDescriptor =
            kafkaSystemDescriptor.getInputDescriptor("wikipedia-raw", new JsonSerdeV2<>(WikipediaFeed.WikipediaFeedEvent.class));
...
//    MessageStream<WikipediaFeed.WikipediaFeedEvent> wikipediaEvents = appDescriptor.getInputStream(wikipediaInputDescriptor);
//    MessageStream<WikipediaFeed.WikipediaFeedEvent> wiktionaryEvents = appDescriptor.getInputStream(wiktionaryInputDescriptor);
//    MessageStream<WikipediaFeed.WikipediaFeedEvent> wikiNewsEvents = appDescriptor.getInputStream(wikiNewsInputDescriptor);
    MessageStream<WikipediaFeed.WikipediaFeedEvent> rawEvents = appDescriptor.getInputStream(rawInputDescriptor);
    OutputStream<WikipediaStatsOutput> wikipediaStats = appDescriptor.getOutputStream(statsOutputDescriptor);

    // Merge inputs
//    MessageStream<WikipediaFeed.WikipediaFeedEvent> allWikipediaEvents =
