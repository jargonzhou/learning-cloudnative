...
public class ByteBufferTest {
    ...
    public void ordering() {
        ...
        byte[] bytes = NIOUtil.toBytes(integer);

        ByteBuffer byteBuffer2 = ByteBuffer.wrap(bytes);
        assertEquals("37fb4c7", NIOUtil.contentOfUnreadBuffer(byteBuffer2));

        byteBuffer2.order(ByteOrder.LITTLE_ENDIAN);
        assertEquals("c7b47f03", NIOUtil.contentOfUnreadBuffer(byteBuffer2));
    }
    ...
    public void charViewOfByteBuffer() {
        ...
        System.out.println(NIOUtil.detailsOfBuffer(byteBuffer));
        System.out.println(NIOUtil.detailsOfBuffer(charBuffer));
    }
