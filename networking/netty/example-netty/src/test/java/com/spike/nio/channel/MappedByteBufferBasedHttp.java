 ...
 * @author zhoujiagen
 */
public class MappedByteBufferBasedHttp {
    ...
    private static final String HTTP_404_HEADER = "HTTP/1.0 404 Not Found" + LINE_SEP + SERVER_ID
            + LINE_SEP;
    ...
    public static void main(String[] args) throws Exception {
        ...
        String file = args[0];
        ByteBuffer headerByteBuffer = ByteBuffer.wrap(NIOUtil.bytes(HTTP_200_HEADER, DEFAULT_CHARSET));
        ...
        try {
            ...
            bb.put(NIOUtil.bytes(message, DEFAULT_CHARSET));
            bb.flip();

            gatherBuffers[0] = ByteBuffer.wrap(NIOUtil.bytes(HTTP_404_HEADER, DEFAULT_CHARSET));
            gatherBuffers[2] = bb;
        ...
        dynamicHeaderByteBuffer.put(NIOUtil.bytes(sb.toString(), DEFAULT_CHARSET));
